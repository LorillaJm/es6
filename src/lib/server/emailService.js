...
// Email Service - Uses Resend (primary) with Gmail SMTP fallback

import { adminDb } from './firebase-admin.js';

// Email configuration from environment variables
let RESEND_API_KEY = '';
let RESEND_FROM = 'Attendance System <onboarding@resend.dev>';
let EMAIL_HOST = 'smtp.gmail.com';
let EMAIL_PORT = '587';
let EMAIL_USER = '';
let EMAIL_PASS = '';
let EMAIL_FROM = 'Attendance System <onboarding@resend.dev>';
let APP_URL = 'https://your-app.vercel.app';
let APP_NAME = 'Attendance System';

try {
    const { env } = await import('$env/dynamic/private');
    RESEND_API_KEY = env.RESEND_API_KEY || '';
    RESEND_FROM = env.RESEND_FROM || 'Attendance System <onboarding@resend.dev>';
    EMAIL_HOST = env.EMAIL_HOST || EMAIL_HOST;
    EMAIL_PORT = env.EMAIL_PORT || EMAIL_PORT;
    EMAIL_USER = env.EMAIL_USER || '';
    EMAIL_PASS = env.EMAIL_PASS || '';
    EMAIL_FROM = env.EMAIL_FROM || EMAIL_FROM;
    APP_URL = env.PUBLIC_APP_URL || APP_URL;
    APP_NAME = env.PUBLIC_APP_NAME || APP_NAME;
} catch (e) {
    console.warn('Email environment variables not available, using defaults');
}

// Determine which email provider to use
const useResend = RESEND_API_KEY && RESEND_API_KEY.startsWith('re_');
console.log(`[EmailService] Using ${useResend ? 'Resend' : 'Gmail SMTP'} for emails`);

// Cached transporter for Gmail SMTP (only used if Resend not configured)
...
async function sendViaResend(options) {
    ...
    try {
        ...
        console.log(`[EmailService] ✅ Resend: Email sent to ${to}`);
...
/**
 * Get or create Gmail SMTP transporter (fallback)
 */
async function getTransporter() {
    ...
    try {
        ...
        console.log('[EmailService] ✅ Gmail SMTP transporter ready');
        
        return cachedTransporter;
    } catch (error) {
        console.error('[EmailService] Gmail SMTP error:', error.message);
...
async function sendViaGmail(options) {
    ...
    try {
        const info = await transporter.sendMail({
            ...
            replyTo: EMAIL_USER,
            headers: {
                'List-Unsubscribe': `<${APP_URL}/app/profile?tab=notifications>`,
                'X-Mailer': `${APP_NAME} Mailer`,
            }
        });

        console.log(`[EmailService] ✅ Gmail: Email sent to ${to}`);
...
/**
 * Send an email (uses Resend if configured, otherwise Gmail)
 */
...
/**
 * Send multiple emails efficiently (with rate limiting for Resend)
 */
export async function sendBulkEmails(emailList) {
    ...
    console.log(`[EmailService] Sending ${emailList.length} emails via ${useResend ? 'Resend' : 'Gmail'}...`);
    const startTime = Date.now();

    const results = [];
    
    if (useResend) {
        // Resend has 2 requests/second limit - send sequentially with delay
        ...
        // Gmail can handle parallel sends
    ...
    console.log(`[EmailService] ✅ Bulk complete: ${sent}/${emailList.length} in ${elapsed}ms`);
    
    if (failed.length > 0) {
        console.warn(`[EmailService] Failed:`, failed.slice(0, 3).map(f => f.error));
    }

    return { sent, failed: failed.length, errors: failed.map(f => f.error) };
}
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f7;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; padding: 32px; border-radius: 0 0 16px 16px;">
            ...
            <div style="text-align: center; margin-bottom: 32px;">
                <div style="display: inline-block; width: 80px; height: 80px; border-radius: 50%; background: linear-gradient(135deg, #4f46e5, #7c3aed); color: white; line-height: 80px; font-size: 28px; font-weight: bold;">
                    ${summary.score}
                </div>
                <p style="margin: 8px 0 0; color: #6b7280; font-size: 12px;">Overall Score</p>
            </div>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
                <tr>
                    <td style="padding: 12px; text-align: center; background: #f9fafb; border-radius: 8px;">
                        <div style="font-size: 20px; font-weight: bold; color: #4f46e5;">${summary.presentDays}/${summary.totalDays}</div>
                        <div style="font-size: 11px; color: #6b7280; margin-top: 4px;">Days Present</div>
                    </td>
                    <td style="width: 12px;"></td>
                    <td style="padding: 12px; text-align: center; background: #f9fafb; border-radius: 8px;">
                        <div style="font-size: 20px; font-weight: bold; color: #059669;">${summary.punctualityRate}%</div>
                        <div style="font-size: 11px; color: #6b7280; margin-top: 4px;">Punctuality</div>
                    </td>
                </tr>
            </table>
            <div style="background: #f9fafb; border-radius: 12px; padding: 16px; margin-bottom: 20px;">
                <table style="width: 100%; font-size: 13px;">
                    <tr><td style="padding: 6px 0; color: #6b7280;">On-time arrivals</td><td style="text-align: right; font-weight: 600; color: #059669;">${summary.onTimeDays} days</td></tr>
                    <tr><td style="padding: 6px 0; color: #6b7280;">Late arrivals</td><td style="text-align: right; font-weight: 600; color: #d97706;">${summary.lateDays} days</td></tr>
                    <tr><td style="padding: 6px 0; color: #6b7280;">Total hours</td><td style="text-align: right; font-weight: 600; color: #1f2937;">${summary.totalHours}h</td></tr>
                </table>
            </div>
        </div>
        <p style="text-align: center; padding: 16px; color: #9ca3af; font-size: 11px;">
            This report was automatically generated. Report ID: ${meta.reportId}
        </p>
    </div>
</body>
</html>`;
